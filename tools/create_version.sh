...
# Change version identifiers inside files from /php + /test + /tools + INFOS, META.json README
	find php test tools INFOS META.json README -type f -exec sed -i "s/<NEXT_VERSION>/${NEW}/g" '{}' \;
...
#???git commit
#???git tag
#?	echo "Don't forget to: git commit -a -m 'Setup new $NEW version'..."
