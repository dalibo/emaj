...
# Retrieve the recently completed rollback operations
  $stmt = $dbh->prepare("SELECT * FROM (SELECT * FROM emaj.emaj_rlbk WHERE rlbk_end_datetime > current_timestamp - '".$complRlbkAgo." hours'::interval ORDER BY rlbk_id DESC LIMIT ".$nbComplRlbk.") AS t ORDER BY rlbk_id ASC");
  $stmt->execute()
...
# Display results
  ...
  while ( $row = $stmt->fetchrow_hashref()) {
	if ($regressTest){
	  ...
	  $row->{'rlbk_start_datetime'} = '<UNDEF>' if (!defined $row->{'rlbk_start_datetime'});
	  $row->{'rlbk_mark_datetime'} = '<UNDEF>' if (!defined $row->{'rlbk_mark_datetime'});
    }
...
Examples:
  perl $PROGRAM -i 3 -n 10
              performs 10 displays during 30 seconds.
  perl $PROGRAM -a 12 -l 10
