...
for (my $i=0 ; $i<$nbSession; $i++){
  print (strftime('%d/%m/%Y - %H:%M:%S',localtime)." Open session #".($i+1)."...\n") if ($verbose);
...
for (my $i=0 ; $i<$nbSession; $i++){
  print (strftime('%d/%m/%Y - %H:%M:%S',localtime)." Start transaction #".($i+1)."...\n") if ($verbose);
  $dbh[$i]->begin_work() 
...
for (my $i=0 ; $i<$nbSession; $i++){
  print (strftime('%d/%m/%Y - %H:%M:%S',localtime)." Call _rlbk_session_lock() for session #".($i+1)." -> lock tables...\n") if ($verbose);
...
for (my $i=0 ; $i<$nbSession; $i++){
  print (strftime('%d/%m/%Y - %H:%M:%S',localtime)." Call _rlbk_session_exec() for session #".($i+1)." -> rollback tables...\n") if ($verbose);
...
for (my $i=0 ; $i<$nbSession; $i++){
  $dbh[$i]->pg_result()
    or die('Get the result of the _rlbk_session_exec() function failed '.$DBI::errstr."\n");
  print (strftime('%d/%m/%Y - %H:%M:%S',localtime)." get result of _rlbk_session_exec call for session #".($i+1)."...\n") if ($verbose);
}
...
# Phase 1 : Prepare transaction
  for (my $i=0 ; $i<$nbSession; $i++){
    print (strftime('%d/%m/%Y - %H:%M:%S',localtime)." Prepare transaction #".($i+1)."...\n") if ($verbose);
    $dbh[$i]->do("PREPARE TRANSACTION 'emajtx".($i+1)."'")
      or die('Prepare transaction #'.($i+1).' failed: '.$DBI::errstr."\n");
  }
# Phase 2 : Commit
  for (my $i=0 ; $i<$nbSession; $i++){
    print (strftime('%d/%m/%Y - %H:%M:%S',localtime)." Commit transaction #".($i+1)."...\n") if ($verbose);
...
Examples:
  $PROGRAM -g myGroup1 -m myMark -s 3
              performs a parallel rollback of table group myGroup1 to mark
              myMark using 3 parallel sessions.
  $PROGRAM -h localhost -p 5432 -d myDb -U emajadm -l -g \"myGroup1,myGroup2\" -m myMark -s 5 -v
