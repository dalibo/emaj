...
select col1, col2, emaj_verb, emaj_tuple, emaj_gid from emaj.myschema4_mypartp1_log;          -- empty in pg 9.6-
select col1, col2, emaj_verb, emaj_tuple, emaj_gid from emaj.myschema4_mypartp2_log;          -- empty in pg 9.6-
...
select cons_group, cons_target_rlbk_mark_name, cons_target_rlbk_mark_time_id, 
       regexp_replace(cons_end_rlbk_mark_name,E'\\d\\d\.\\d\\d\\.\\d\\d\\.\\d\\d\\d','%','g'), cons_end_rlbk_mark_time_id, cons_rows, cons_marks 
...
select mark_group, regexp_replace(mark_name,E'\\d\\d\.\\d\\d\\.\\d\\d\\.\\d\\d\\d','%','g'), mark_time_id, mark_is_deleted, mark_is_rlbk_protected, mark_comment, mark_log_rows_before_next, mark_logged_rlbk_target_mark from emaj.emaj_mark where mark_group = 'myGroup1' order by mark_time_id;
...
begin;
  ...
  select * from emaj.emaj_mark where mark_group = 'emptyGroup' order by mark_time_id;
rollback;
