...
The command::
   ...
   emajParallelRollback.pl -d mydb -g myGroup1 -m M1 -s 3

logs into database *mydb* and executes a rollback of group *myGroup1* to mark *M1*, using 3 parallel sessions.

The command::

   emajParallelRollback.pl -d mydb -g "myGroup1,myGroup2" -m M1 -s 3 -l

logs into database *mydb* and executes a logged rollback of both groups *myGroup1* and *myGroup2* to mark *M1*, using 3 parallel sessions.
