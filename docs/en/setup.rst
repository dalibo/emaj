...
.. caution::
   ...
   It is not mandatory to execute the installation script as *SUPERUSER*. But if it is not the case, some functionnalities are missing, depending on the effective rights owned by the installer role. These limitations and their possible mitigations are :ref:`detailed here<non_superuser_install_limits>`.
...
Two configuration parameters may need to be changed in the postgresql.conf file:
  ...
  * **max_locks_per_transaction** to manage table groups with a high number of tables. Main E-Maj functions set a lock on each table of a processed table group. If an E-Maj operation fails with a message indicating that all entries of the *shared lock table* have been used, this parameter must be increased. Its default value equals 128 (as of PostgreSQL 19 and 64 in earlier versions).
...
This drops the *emaj_demo_app_schema* schema and both *emaj demo group 1* and *emaj demo group 2* table groups.
