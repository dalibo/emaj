...
If the extension is installed by a non *SUPERUSER* role, this role (and only this role) must be granted the right to execute the *dblink_connect_u(text,text)* function, this right not been granted by default for security reasons. It will allow to :ref:`monitor E-Maj rollbacks<emaj_rollback_activity_prerequisites>` or :doc:`submit parallel E-Maj rollbacks<parallelRollbackClient>`::

   GRANT EXECUTE ON FUNCTION dblink_connect_u(text,text) TO <install_role>;
