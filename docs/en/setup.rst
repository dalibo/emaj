...
- **max_locks_per_transaction**:
  E-Maj functions set a lock on each table in a processed table group. If an E-Maj operation fails with a message indicating that all entries in the *shared lock table* are exhausted, increase this parameter.
  Default value: **128** (PostgreSQL 19+) or **64** (earlier versions).

- **max_prepared_transactions**:
  Required to use the :doc:`E-Maj parallel rollback client <parallelRollbackClient>`.
  Default value: **0** (disables the tool). This value must be **greater than or equal to** the maximum number of concurrent parallel rollback operations.
...
Setting these parameters is **optional**, as E-Maj works well with default values. However, in order to use the :ref:`rollback operations monitoring capabilities <emaj_rollback_activity>`, or execute :doc:`parallel E-Maj rollbacks <parallelRollbackClient>`, you **must** set the ``dblink_user_password`` parameter.
