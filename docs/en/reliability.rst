...
Two additional elements help in ensuring the E-Maj reliability: internal checks are performed at some key moments of table groups life and event trigers can block some risky operations.
...
When a function is executed to start a table group, to set a mark or to rollback a table group, E-Maj performs some checks in order to verify the integrity of the table groups to process.
...
By using the :ref:`emaj_verify_all() <emaj_verify_all>` function, the administrator can perform the same checks on demand on all table groups.
