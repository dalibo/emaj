...
Exporting an E-Maj Parameters Configuration
-------------------------------------------
...
- ``p_mark`` (*TEXT*, optional): **Mark** set on table groups in **LOGGING** state. It may contain a generic ``%`` character, which is replaced by the current time with the pattern ``hh.mm.ss.mmmm``. If the parameter is not specified, or is empty or *NULL*, a name is automatically generated: ``IMPORT_%``.
...
If an existing table group is missing from the configuration or is not listed in groups to be imported, it remains unchanged by default. However, if the ``p_dropOtherGroups`` parameter is set to *TRUE*, this group is dropped, regardless of its state.

----

.. _import_param_conf:

Importing an E-Maj Parameters Configuration
-------------------------------------------
