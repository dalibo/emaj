...
Il est aisé d'exécuter des requêtes plus précises sur ces statistiques. Par exemple, on peut obtenir le nombre de mises à jour par schéma applicatif avec une requête du type :

.. code-block:: sql

   SELECT stat_schema, sum(stat_rows)
   FROM emaj.emaj_log_stat_group('mon_groupe', 'mark_1', NULL)
...
On peut obtenir les statistiques pour **une seule table** sur un **intervalle de temps** donné avec l’une des deux requêtes SQL : ::
...
- ``p_schema`` (*TEXT*) : **Schéma** contenant la table à examiner.
- ``p_table`` (*TEXT*) : Nom de la **table** à examiner.
- ``p_startTs`` (*TIMESTAMPTZ*, optionnel, par défaut *NULL*) : **Date et heure du début** de la période d'examen. *NULL* signifie que l'examen démarre aux plus anciennes données disponibles pour la table.
- ``p_endTs`` (*TIMESTAMPTZ*, optionnel, par défaut *NULL*) : **Date et heure du fin** de la période d'examen. *NULL* signifie que l'examen se termine sur l'état courant de la table.
- ``p_startGroup`` (*TEXT*) : **Groupe** de tables de la marque de **début** de période d'examen.
- ``p_startMark`` (*TEXT*) : Nom de la **marque** de **début** de période d'examen. *NULL* signifie que l'examen démarre aux plus anciennes données disponibles pour la table.
- ``p_endGroup`` (*TEXT*, optionnel, par défaut *NULL*) : **Groupe** de tables de la marque de **fin** de période d'examen.
- ``p_endMark`` (*TEXT*, optionnel, par défaut *NULL*) : Nom de la **marque** de **fin** de période d'examen. *NULL* signifie que l'examen se termine sur l'état courant de la table.
...
- ``p_schema`` (*TEXT*) : **Schéma** contenant la séquence à examiner.
- ``p_sequence`` (*TEXT*) : Nom de la **séquence** à examiner.
- ``p_startTs`` (*TIMESTAMPTZ*, optionnel, par défaut *NULL*) : **Date et heure du début** de la période d'examen. *NULL* signifie que l'examen démarre aux plus anciennes données disponibles pour la séquence.
- ``p_endTs`` (*TIMESTAMPTZ*, optionnel, par défaut *NULL*) : **Date et heure du fin** de la période d'examen. *NULL* signifie que l'examen se termine sur l'état courant de la séquence.
- ``p_startGroup`` (*TEXT*) : **Groupe** de tables de la marque de **début** de période d'examen.
- ``p_startMark`` (*TEXT*) : Nom de la **marque** de **début** de période d'examen. *NULL* signifie que l'examen démarre aux plus anciennes données disponibles pour la séquence.
- ``p_endGroup`` (*TEXT*, optionnel, par défaut *NULL*) : **Groupe** de tables de la marque de **fin** de période d'examen.
- ``p_endMark`` (*TEXT*, optionnel, par défaut *NULL*) : Nom de la **marque** de **fin** de période d'examen. *NULL* signifie que l'examen se termine sur l'état courant de la séquence.
