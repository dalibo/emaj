...
* 2 rôles de type groupe (sans possibilité de connexion) :

  - ``emaj_adm`` pour administrer les composants E-Maj,
  - ``emaj_viewer`` pour uniquement consulter les composants E-Maj,
...
.. note::
  Les **schémas de log** sont gérées exclusivement par les fonctions E-Maj. Ils ne **doivent pas** contenir d'objets autres que ceux créés par E-Maj.
...
Pour une table applicative, le nom des objets de log est préfixé par le nom de la table :

* **table de log** :  ``<nom.de.la.table>_log``

* **fonction de log** : ``<nom.de.la.table>_log_fnct``

* **séquence** associée à la table de log : ``<nom.de.la.table>_log_seq``
...
* ``emaj_log_trg`` pour les triggers de log des *INSERT*, *UPDATE*, *DELETE*,
