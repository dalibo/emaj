...
Il est également nécessaire de valoriser le paramètre E-Maj :doc:`dblink_user_password<parameters>`.
...
La commande : ::
   ...
   emajParallelRollback.pl -d maBase -g monGroupe1 -m M1 -s 3

se connecte à la base de données *mabase* et exécute un rollback du groupe *monGroupe1* à la marque *M1*, avec 3 sessions en parallèle.

La commande : ::

   emajParallelRollback.pl -d maBase -g "monnGroupe1,monGroupe2" -m M1 -s 3 -l

se connecte à la base de données *maBase* et exécute un rollback tracé (« *logged rollback* ») des 2 groupes *monGroupe1* et *monGroupe2* à la marque *M1*, avec 3 sessions en parallèle.
