...
-- Setup the install configuration characteristics.
DO LANGUAGE plpgsql
$do$
-- The v_createdAsExtension constant is set to FALSE in the psql creation script (generated by the create_sql_install_script.pl tool).
  DECLARE
    ...
    v_supportEventTriggers   BOOLEAN = TRUE;
  BEGIN
...
-- Store the installation characteristics into the emaj_install_conf table and protect its content.
    INSERT INTO emaj.emaj_install_conf (inst_as_extension, inst_by_superuser, inst_with_event_triggers)
      VALUES (v_createdAsExtension, v_isRoleSuperuser, v_supportEventTriggers);
...
-- Get the installation configuration data.
    SELECT inst_by_superuser, inst_with_event_triggers
      INTO STRICT v_installedBySuperuser, v_supportEventTriggers
      FROM emaj.emaj_install_conf;
