...
CREATE or REPLACE FUNCTION emaj._log_stat_table(v_schemaName TEXT, v_tableName TEXT, v_logSchema TEXT, v_tsFirstMark TIMESTAMPTZ, v_tsLastMark TIMESTAMPTZ, v_firstLastSeqHoleId BIGINT, v_lastLastSeqHoleId BIGINT) 
...
-- check the application relation exist
    SELECT relkind INTO v_realRelKind FROM pg_class, pg_namespace
      WHERE relnamespace = pg_namespace.oid 
...
--   -> check boths functions exists
          PERFORM proname FROM pg_proc , pg_namespace WHERE 
            pronamespace = pg_namespace.oid AND nspname = v_logSchema AND proname = v_logFnctName;
          IF NOT FOUND THEN
            RETURN NEXT 'log function ' || v_logFnctName || ' not found.';
          END IF;
          IF v_isRollbackable THEN
            PERFORM proname FROM pg_proc, pg_namespace WHERE 
              pronamespace = pg_namespace.oid AND nspname = v_logSchema AND proname = v_rlbkFnctName;
            IF NOT FOUND THEN
...
--   -> check the log table exists
          PERFORM relname FROM pg_class, pg_namespace WHERE 
            relnamespace = pg_namespace.oid AND nspname = v_logSchema AND relkind = 'r' AND relname = v_logTableName;
          IF NOT FOUND THEN
...
-- check that the schema doesn't already exist
      PERFORM 0 FROM pg_namespace WHERE nspname = r_schema.log_schema;
      IF FOUND THEN
        RAISE EXCEPTION 'emaj_create_group: schema % should not exist. Drop it manually, or modify emaj_group_def table''s content.',r_schema.log_schema;
      END IF;
...
-- and record the schema creation in emaj_hist table
      INSERT INTO emaj.emaj_hist (hist_function, hist_event, hist_object) 
        VALUES ('CREATE_GROUP','SECONDARY SCHEMA CREATED',quote_ident(r_schema.log_schema));
    END LOOP;
...
-- for each table of the emaj_relation table, get the number of log rows and return the statistic
    FOR r_tblsq IN
        SELECT rel_priority, rel_schema, rel_tblseq, emaj._log_stat_table(rel_schema, rel_tblseq, rel_log_schema, v_tsFirstMark, v_tsLastMark, v_firstLastSeqHoleId, v_lastLastSeqHoleId) AS nb_rows FROM emaj.emaj_relation 
          WHERE rel_group = v_groupName AND rel_kind = 'r' ORDER BY rel_priority, rel_schema, rel_tblseq
        LOOP
