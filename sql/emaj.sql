...
CREATE OR REPLACE FUNCTION emaj._create_tbl(grpdef emaj.emaj_group_def, v_groupName TEXT, v_isRollbackable BOOLEAN, v_defTsp TEXT)
RETURNS void LANGUAGE plpgsql SECURITY DEFINER AS
$_create_tbl$
...
-- check the table is neither a temporary nor an unlogged table
    IF v_pgVersion >= '9.1' THEN
      SELECT relpersistence INTO v_relPersistence FROM pg_catalog.pg_class, pg_catalog.pg_namespace
        WHERE relnamespace = pg_namespace.oid AND nspname = grpdef.grpdef_schema AND relname = grpdef.grpdef_tblseq;
      IF v_relPersistence = 't' THEN
        RAISE EXCEPTION '_create_tbl: table % is a temporary table.', grpdef.grpdef_tblseq;
      ELSIF v_relPersistence = 'u' THEN
        RAISE EXCEPTION '_create_tbl: table % is an unlogged table.', grpdef.grpdef_tblseq;
      END IF;
    ...
    IF v_isRollbackable AND v_relhaspkey = FALSE THEN
      RAISE EXCEPTION '_create_tbl: table % has no PRIMARY KEY.', grpdef.grpdef_tblseq;
    END IF;
-- check the table has a primary key
    SELECT true INTO v_relhaspkey FROM pg_catalog.pg_class, pg_catalog.pg_namespace, pg_catalog.pg_constraint
      WHERE relnamespace = pg_namespace.oid AND connamespace = pg_namespace.oid AND conrelid = pg_class.oid
        AND contype = 'p' AND nspname = grpdef.grpdef_schema AND relname = grpdef.grpdef_tblseq;
    IF NOT FOUND THEN
      v_relhaspkey = false;
    END IF;
    IF v_isRollbackable AND v_relhaspkey = FALSE THEN
...
-- prepare TABLESPACE clauses for data and index
    v_logDatTsp = coalesce(grpdef.grpdef_log_dat_tsp, v_defTsp);
    v_logIdxTsp = coalesce(grpdef.grpdef_log_idx_tsp, v_defTsp);
...
-- build the different name for table, trigger, functions,...
    v_logSchema        = coalesce(v_schemaPrefix || grpdef.grpdef_log_schema_suffix, v_emajSchema);
    v_fullTableName    = quote_ident(grpdef.grpdef_schema) || '.' || quote_ident(grpdef.grpdef_tblseq);
    v_logTableName     = quote_ident(v_logSchema) || '.' || quote_ident(grpdef.grpdef_schema || '_' || grpdef.grpdef_tblseq || '_log');
    v_logIdxName       = quote_ident(grpdef.grpdef_schema || '_' || grpdef.grpdef_tblseq || '_log_idx');
    v_logFnctName      = quote_ident(v_logSchema) || '.' || quote_ident(grpdef.grpdef_schema || '_' || grpdef.grpdef_tblseq || '_log_fnct');
    v_logTriggerName   = quote_ident(grpdef.grpdef_schema || '_' || grpdef.grpdef_tblseq || '_emaj_log_trg');
    v_truncTriggerName = quote_ident(grpdef.grpdef_schema || '_' || grpdef.grpdef_tblseq || '_emaj_trunc_trg');
    v_sequenceName     = quote_ident(v_logSchema) || '.' || quote_ident(emaj._build_log_seq_name(grpdef.grpdef_schema, grpdef.grpdef_tblseq));
    ...
    FOR r_column IN
      SELECT ' ALTER COLUMN ' || quote_ident(attname) || ' DROP NOT NULL' AS action
        ...
        WHERE relnamespace = pg_namespace.oid AND attrelid = pg_class.oid
          AND nspname = v_logSchema AND relname = grpdef.grpdef_schema || '_' || grpdef.grpdef_tblseq || '_log'
...
-- register the table into emaj_relation
    INSERT INTO emaj.emaj_relation (rel_schema, rel_tblseq, rel_group, rel_priority, rel_log_schema, rel_log_dat_tsp, rel_log_idx_tsp, rel_kind)
        VALUES (grpdef.grpdef_schema, grpdef.grpdef_tblseq, grpdef.grpdef_group, grpdef.grpdef_priority, v_logSchema, v_logDatTsp, v_logIdxTsp, 'r');
--
...
-- Required inputs: the emaj_group_def row related to the application table to process, the group name
  DECLARE
    v_tableSchema            TEXT;
    v_tableName              TEXT;
    v_tableGroup             TEXT;
