-- emaj_set_event_triggers_protection.sql
-- This psql script belongs to the E-Maj extension
--
-- It (re)installs the components that are needed to setup the event trigger protection. These components are also created by
-- the standart installation procedure. But their effective creation depends on the postgres version used at installation time
-- because the event trigger feature doesn't exist in the oldest postgres versions supported by E-Maj.
-- So the script has to be used when E-Maj was initially installed with a postgres version prior 9.5 and a postgres version
-- upgrade leads to a more recent version.
--
...
-- add both event trigger components to the emaj extension
ALTER EXTENSION emaj ADD FUNCTION emaj._event_trigger_table_rewrite_fnct();
EXECUTE '
