...
--
-- In the previous version, it only managed the components that are needed to setup the event trigger protection. These components are also created
...
-- Now that the extension has to be executed with postgres 9.5+, there should not be any missing event triggers.
-- But the code is kept here, in case of... It will be probably deleted in a coming version.
--
...
-- the installed postgres version must be at least 9.5
    IF current_setting('server_version_num')::int < 90500 THEN
      RAISE EXCEPTION 'E-Maj upgrade: the current PostgreSQL version (%) is not compatible with E-Maj <devel>. The PostgreSQL version should be at least 9.5.', current_setting('server_version');
    END IF;
  END;
$do$;
