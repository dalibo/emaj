...
---------------------------------------
--
-- create the new emaj_time_stamp table and populate it
--
...
CREATE TABLE emaj.emaj_time_stamp (
  ...
  time_event                   CHAR(1),                    -- event type that has generated the time stamp
                                                           --   C(reate), A(lter), D(rop) , M(ark), R(ollback)
  PRIMARY KEY (time_id)
  );
COMMENT ON TABLE emaj.emaj_time_stamp IS
$$Contains the time stamps of major E-Maj events.$$;

-- create a temporary table with the different time stamp to record
CREATE TEMP TABLE emaj_temp_time_stamp (
  tmp_ts                      TIMESTAMPTZ,
  tmp_event                   CHAR(1)
  );

-- get all known timestamp from groups, marks and rollbacks
INSERT INTO emaj_temp_time_stamp
  SELECT group_creation_datetime, 'C' FROM emaj.emaj_group;
INSERT INTO emaj_temp_time_stamp
  SELECT group_last_alter_datetime, 'A' FROM emaj.emaj_group WHERE group_last_alter_datetime IS NOT NULL;
INSERT INTO emaj_temp_time_stamp
  SELECT mark_datetime, 'M' FROM emaj.emaj_mark;
INSERT INTO emaj_temp_time_stamp
  SELECT rlbk_start_datetime, 'R' FROM emaj.emaj_rlbk;

-- build the emaj_time_stamp table
INSERT INTO emaj.emaj_time_stamp (time_clock_timestamp, time_stmt_timestamp, time_tx_timestamp, time_tx_id, time_event)
  SELECT DISTINCT ON (tmp_ts, tmp_event) tmp_ts, tmp_ts, tmp_ts, NULL, tmp_event
    FROM emaj_temp_time_stamp
    ORDER BY tmp_ts, tmp_event;
-- copy the known global sequence value for each mark
UPDATE emaj.emaj_time_stamp SET time_last_emaj_gid = mark_global_seq
  FROM emaj.emaj_mark
  WHERE mark_datetime = time_clock_timestamp;
...
--
-- create a temporary table with the old structure and copy the source content
CREATE TEMP TABLE emaj_mark_old (LIKE emaj.emaj_mark);

INSERT INTO emaj_mark_old SELECT * FROM emaj.emaj_mark;

-- drop the old table
ALTER EXTENSION emaj DROP SEQUENCE emaj.emaj_mark_mark_id_seq;
DROP TABLE emaj.emaj_mark CASCADE;
...
SELECT CASE WHEN EXISTS (SELECT 1 FROM emaj.emaj_mark) 
              THEN setval('emaj.emaj_mark_mark_id_seq', (SELECT max(mark_id) FROM emaj.emaj_mark))
       END;

--
-- process the emaj_sequence table
--
-- create a temporary table with the old structure and copy the source content
CREATE TEMP TABLE emaj_sequence_old (LIKE emaj.emaj_sequence);

INSERT INTO emaj_sequence_old SELECT * FROM emaj.emaj_sequence;

-- drop the old table
ALTER EXTENSION emaj DROP SEQUENCE emaj.emaj_sequence_sequ_id_seq;
DROP TABLE emaj.emaj_sequence CASCADE;
...
--
-- create a temporary table with the old structure and copy the source content
CREATE TEMP TABLE emaj_seq_hole_old (LIKE emaj.emaj_seq_hole);

INSERT INTO emaj_seq_hole_old SELECT * FROM emaj.emaj_seq_hole;

-- drop the old table
ALTER EXTENSION emaj DROP SEQUENCE emaj.emaj_seq_hole_sqhl_id_seq;
DROP TABLE emaj.emaj_seq_hole;
...
CREATE TABLE emaj.emaj_seq_hole (
  ...
  sqhl_begin_mark_id           BIGINT      NOT NULL,       -- id of the mark that represents the beginning of the hole
                                                           --   (mark_ids order follows time order)
  sqhl_begin_mark_ts           TIMESTAMPTZ,                -- timestamp of the mark that represents the beginning of the hole (for information only)
  sqhl_end_mark_id             BIGINT      NOT NULL,       -- id of the mark that represents the end of the hole
                                                           --   (when the hole is produced by an unlogged rollback operation, it is just a
                                                           --   "pseudo mark" that is generated by incrementing the sequence linked to mark_id)
  sqhl_end_mark_ts             TIMESTAMPTZ,                -- timestamp of the mark that represents the end of the hole (for information only)
  sqhl_hole_size               BIGINT      NOT NULL,       -- hole size computed as the difference of 2 sequence last-values
  PRIMARY KEY (sqhl_schema, sqhl_table, sqhl_begin_mark_id)
  );
COMMENT ON TABLE emaj.emaj_seq_hole IS
$$Contains description of holes in sequence values for E-Maj log tables.$$;

-- populate the new table
-- The emaj_seq_hole structure change needs a quite complex processing, using data also from emaj_mark and emaj_rlbk tables
--TODO : Tests unitaires puis ménage dans les instructions de debug
DO
$do$
  DECLARE
    v_minMarkTs              TIMESTAMPTZ;
    v_endMarkId              BIGINT;
    v_nbRows                 INTEGER;
    v_sumRowsIn              INTEGER;
    v_sumRowsOt              INTEGER = 0;
    r_rlbk                   RECORD;
    r_group                  RECORD;
  BEGIN
-- get the oldest mark datetime (to limit the search among rollback operations)
    SELECT min(mark_datetime) INTO v_minMarkTs FROM emaj.emaj_mark;
----TODO raise notice 'la plus vieille marque date de %',v_minMarkTs;
-- for each unlogged rollback operation already performed on the database with a not too old target mark 
    FOR r_rlbk IN
      SELECT rlbk_groups, rlbk_mark, rlbk_mark_datetime, rlbk_start_datetime FROM emaj.emaj_rlbk
        WHERE NOT rlbk_is_logged AND
              rlbk_mark_datetime >= v_minMarkTs
        ORDER BY rlbk_id DESC
      LOOP
----TODO raise notice '> Un rollback : groupes %, mark %, date mark %, date rlbk %', r_rlbk.rlbk_groups, r_rlbk.rlbk_mark, r_rlbk.rlbk_mark_datetime, r_rlbk.rlbk_start_datetime;
-- ... compute the future id of the pseudo end rollback mark
        SELECT min(mark_id) INTO v_endMarkId FROM emaj.emaj_mark
          WHERE mark_datetime > r_rlbk.rlbk_mark_datetime;
----TODO raise notice 'id pseudo mark à créer = %', v_endMarkId;
-- ... create a hole in the mark id sequence just after the time when the rollback operation was done to mimic the future behaviour of these operations
--   increment the mark_id of all marks set after the rollback
        UPDATE emaj.emaj_mark SET mark_id = mark_id + 1
          WHERE mark_datetime > r_rlbk.rlbk_mark_datetime;
----TODO GET DIAGNOSTICS v_nbRows = ROW_COUNT;
----TODO raise notice '=> % marks modifiées', v_nbRows;
--   and the sequence of the mark_id serial
        PERFORM nextval('emaj.emaj_mark_mark_id_seq');
-- ... build the corresponding rows in the emaj_seq_hole table
--   for each group processed by the rollback operation (there may be several)
        FOR r_group IN
          SELECT mark_id, mark_group FROM emaj.emaj_mark
            WHERE mark_name = r_rlbk.rlbk_mark AND mark_group = ANY (r_rlbk.rlbk_groups)
          LOOP
----TODO raise notice '  >> Un groupe à traiter % - id = %', r_group.mark_group, r_group.mark_id;
--   insert the rows for tables belonging to the group
          INSERT INTO emaj.emaj_seq_hole (sqhl_schema, sqhl_table, sqhl_begin_mark_id, sqhl_begin_mark_ts, sqhl_end_mark_id, sqhl_end_mark_ts, sqhl_hole_size)
            SELECT sqhl_schema, sqhl_table, r_group.mark_id, r_rlbk.rlbk_mark_datetime, v_endMarkId, r_rlbk.rlbk_start_datetime, sqhl_hole_size
              FROM emaj_seq_hole_old, emaj.emaj_relation
              WHERE sqhl_schema = rel_schema AND sqhl_table = rel_tblseq AND rel_group = r_group.mark_group
                AND sqhl_datetime = r_rlbk.rlbk_mark_datetime;
          GET DIAGNOSTICS v_nbRows = ROW_COUNT;
----TODO raise notice '=> % lignes copiées', v_nbRows;
          v_sumRowsOt = v_sumRowsOt + v_nbRows;
        END LOOP;
    END LOOP;
-- check all rows of the original emaj_seq_hole have been migrated
    SELECT count(*) INTO v_sumRowsIn FROM emaj_seq_hole_old;
----TODO raise notice 'nb lignes in % => ot %', v_sumRowsIn, v_sumRowsOt;
    IF v_sumRowsOt <> v_sumRowsIn THEN
----TODO      RAISE EXCEPTION 'E-Maj upgrade: internal error, the number of rows in the original emaj_seq_hole (%) doesn''t math the number of rows in the new emaj_seq_hole table (%)', v_sumRowsIn, v_sumRowsOt;
      RAISE NOTICE 'E-Maj upgrade: internal error, the number of rows in the original emaj_seq_hole (%) doesn''t math the number of rows in the new emaj_seq_hole table (%)', v_sumRowsIn, v_sumRowsOt;
    END IF;
  END;
$do$;
...
INSERT INTO emaj.emaj_rlbk (
         rlbk_id, rlbk_groups, rlbk_mark, rlbk_mark_datetime, rlbk_time_id, rlbk_is_logged,
         rlbk_nb_session, rlbk_nb_table, rlbk_nb_sequence, rlbk_eff_nb_table, rlbk_status, 
         rlbk_begin_hist_id, rlbk_is_dblink_used, rlbk_start_datetime, rlbk_end_datetime, rlbk_msg)
  SELECT rlbk_id, rlbk_groups, rlbk_mark, rlbk_mark_datetime, time_id, rlbk_is_logged,
         rlbk_nb_session, rlbk_nb_table, rlbk_nb_sequence, rlbk_eff_nb_table, rlbk_status, 
         rlbk_begin_hist_id, rlbk_is_dblink_used, rlbk_start_datetime, rlbk_end_datetime, rlbk_msg
    FROM emaj_rlbk_old, emaj.emaj_time_stamp
    WHERE rlbk_start_datetime = time_clock_timestamp AND time_event = 'R';
...
--
-- Check the number of rows in old and new tables and drop old versions of tables
--
DO
$do$
  DECLARE
    ...
    v_nbRowsNew       INT;
  BEGIN
    SELECT count(*) INTO v_nbRowsOld FROM emaj_group_old;
    SELECT count(*) INTO v_nbRowsNew FROM emaj.emaj_group;
    IF v_nbRowsOld <> v_nbRowsNew THEN
      RAISE EXCEPTION 'E-Maj upgrade: internal error, the number of rows in the original emaj_group table (%) doesn''t match the number of rows in the new emaj_group table (%)',v_nbRowsOld, v_nbRowsNew;
    END IF;
    SELECT count(*) INTO v_nbRowsOld FROM emaj_mark_old;
    SELECT count(*) INTO v_nbRowsNew FROM emaj.emaj_mark;
    IF v_nbRowsOld <> v_nbRowsNew THEN
      RAISE EXCEPTION 'E-Maj upgrade: internal error, the number of rows in the original emaj_mark table (%) doesn''t match the number of rows in the new emaj_mark table (%)',v_nbRowsOld, v_nbRowsNew;
    END IF;
    SELECT count(*) INTO v_nbRowsOld FROM emaj_sequence_old;
    SELECT count(*) INTO v_nbRowsNew FROM emaj.emaj_sequence;
    IF v_nbRowsOld <> v_nbRowsNew THEN
      RAISE EXCEPTION 'E-Maj upgrade: internal error, the number of rows in the original emaj_sequence table (%) doesn''t match the number of rows in the new emaj_sequence table (%)',v_nbRowsOld, v_nbRowsNew;
    END IF;
    SELECT count(*) INTO v_nbRowsOld FROM emaj_seq_hole_old;
    SELECT count(*) INTO v_nbRowsNew FROM emaj.emaj_seq_hole;
--    IF v_nbRowsOld <> v_nbRowsNew THEN
--      RAISE EXCEPTION 'E-Maj upgrade: internal error, the number of rows in the original emaj_seq_hole table (%) doesn''t match the number of rows in the new emaj_seq_hole table (%)',v_nbRowsOld, v_nbRowsNew;
--    END IF;
    SELECT count(*) INTO v_nbRowsOld FROM emaj_rlbk_old;
    SELECT count(*) INTO v_nbRowsNew FROM emaj.emaj_rlbk;
    IF v_nbRowsOld <> v_nbRowsNew THEN
      RAISE EXCEPTION 'E-Maj upgrade: internal error, the number of rows in the original emaj_rlbk table (%) doesn''t match the number of rows in the new emaj_rlbk table (%)',v_nbRowsOld, v_nbRowsNew;
    END IF;
    SELECT count(*) INTO v_nbRowsOld FROM emaj_rlbk_stat_old;
    SELECT count(*) INTO v_nbRowsNew FROM emaj.emaj_rlbk_stat;
    IF v_nbRowsOld <> v_nbRowsNew THEN
      RAISE EXCEPTION 'E-Maj upgrade: internal error, the number of rows in the original emaj_rlbk_stat table (%) doesn''t match the number of rows in the new emaj_rlbk_stat table (%)',v_nbRowsOld, v_nbRowsNew;
    END IF;
  END;
$do$;

DROP TABLE emaj_group_old;
DROP TABLE emaj_mark_old;
DROP TABLE emaj_sequence_old;
DROP TABLE emaj_seq_hole_old;
...
DROP FUNCTION emaj.emaj_create_group(V_GROUPNAME TEXT);
DROP FUNCTION emaj.emaj_start_group(V_GROUPNAME TEXT,V_MARK TEXT);
DROP FUNCTION emaj.emaj_start_groups(V_GROUPNAMES TEXT[],V_MARK TEXT);
DROP FUNCTION emaj.emaj_stop_group(V_GROUPNAME TEXT);
DROP FUNCTION emaj.emaj_stop_groups(V_GROUPNAMES TEXT[]);
DROP FUNCTION emaj._set_mark_groups(V_GROUPNAMES TEXT[],V_MARK TEXT,V_MULTIGROUP BOOLEAN,V_EVENTTORECORD BOOLEAN);
DROP FUNCTION emaj.emaj_delete_mark_group(V_GROUPNAME TEXT,V_MARK TEXT);
DROP FUNCTION emaj.emaj_delete_before_mark_group(V_GROUPNAME TEXT,V_MARK TEXT);
DROP FUNCTION emaj._delete_before_mark_group(V_GROUPNAME TEXT,V_MARK TEXT);
DROP FUNCTION emaj.emaj_gen_sql_group(V_GROUPNAME TEXT,V_FIRSTMARK TEXT,V_LASTMARK TEXT,V_LOCATION TEXT);
DROP FUNCTION emaj.emaj_gen_sql_group(V_GROUPNAME TEXT,V_FIRSTMARK TEXT,V_LASTMARK TEXT,V_LOCATION TEXT,V_TBLSEQS TEXT[]);
DROP FUNCTION emaj.emaj_gen_sql_groups(V_GROUPNAMES TEXT[],V_FIRSTMARK TEXT,V_LASTMARK TEXT,V_LOCATION TEXT);
DROP FUNCTION emaj.emaj_gen_sql_groups(V_GROUPNAMES TEXT[],V_FIRSTMARK TEXT,V_LASTMARK TEXT,V_LOCATION TEXT,V_TBLSEQS TEXT[]);
...
$$
-- This function returns the creation clock timestamp of a mark if exists for a group,
...
SELECT CASE
         WHEN $2 = 'EMAJ_LAST_MARK' THEN
              (SELECT time_clock_timestamp FROM emaj.emaj_mark, emaj.emaj_time_stamp
                 WHERE time_id = mark_time_id AND mark_group = $1 ORDER BY time_id DESC LIMIT 1)
         ELSE (SELECT time_clock_timestamp FROM emaj.emaj_mark, emaj.emaj_time_stamp
                 WHERE time_id = mark_time_id AND mark_group = $1 AND mark_name = $2)
       END
$$;
...
CREATE OR REPLACE FUNCTION emaj._delete_log_tbl(r_rel emaj.emaj_relation, v_beginMarkId BIGINT, v_beginMarkTimeId BIGINT, v_beginMarkTs TIMESTAMPTZ, v_endMarkId BIGINT, v_endMarkTs TIMESTAMPTZ, v_lastGlobalSeq BIGINT, v_lastSequenceId BIGINT)
...
-- The function is called by emaj._rlbk_session_exec()
-- Input: row from emaj_relation corresponding to the appplication table to proccess,
--        begin and end mark ids and mark time stamp ids to define the marks range identifying the hole that will be created in the log sequence
...
-- first delete, if exist, sequence holes that have disappeared with the rollback
    DELETE FROM emaj.emaj_seq_hole
      WHERE sqhl_schema = r_rel.rel_schema AND sqhl_table = r_rel.rel_tblseq
        AND sqhl_begin_mark_id >= v_beginMarkId AND sqhl_begin_mark_id < v_endMarkId;
-- and then insert the new sequence hole
    EXECUTE 'INSERT INTO emaj.emaj_seq_hole (sqhl_schema, sqhl_table, sqhl_begin_mark_id, sqhl_begin_mark_ts, '
      || 'sqhl_end_mark_id, sqhl_end_mark_ts, sqhl_hole_size) VALUES ('
      || quote_literal(r_rel.rel_schema) || ',' || quote_literal(r_rel.rel_tblseq) || ',' || v_beginMarkId || ',' || quote_literal(v_beginMarkTs)
      || ',' || v_endMarkId || ',' || quote_literal(v_endMarkTs) || ', ('
      ...
      || ' AND sequ_time_id = ' || v_beginMarkTimeId || '))';
    RETURN v_nbRows;
  END;
$_delete_log_tbl$;
...
CREATE OR REPLACE FUNCTION emaj._log_stat_tbl(r_rel emaj.emaj_relation, v_firstMarkId BIGINT, v_lastMarkId BIGINT, v_firstMarkTimeId BIGINT, v_lastMarkTimeId BIGINT)
RETURNS BIGINT LANGUAGE plpgsql AS
$_log_stat_tbl$
...
-- Input: schema name and table name, log schema, the time stamp ids of both marks, the mark id of both marks
--   a time stamp id of last mark set to NULL indicates the current situation
...
--   and count the sum of hole from the start mark time until now
      SELECT coalesce(sum(sqhl_hole_size),0) INTO v_sumHole FROM emaj.emaj_seq_hole
        WHERE sqhl_schema = r_rel.rel_schema AND sqhl_table = r_rel.rel_tblseq
          AND sqhl_begin_mark_id >= v_firstMarkId;
    ELSE
...
--   and count the sum of hole from the begin mark time to the end mark time
      SELECT coalesce(sum(sqhl_hole_size),0) INTO v_sumHole FROM emaj.emaj_seq_hole
        WHERE sqhl_schema = r_rel.rel_schema AND sqhl_table = r_rel.rel_tblseq
          AND sqhl_begin_mark_id >= v_firstMarkId AND sqhl_end_mark_id <= v_lastMarkId;
    END IF;
...
-- The function is called by the emaj_delete_before_mark_group(), emaj_delete_mark_group() and emaj_consolidate_rollback_group() functions.
...
--        when the first mark is NULL, it represents the very first known mark.
-- Output: number of deleted marks, number of tables effectively processed (for which at least one log row has been deleted)
  DECLARE
    ...
    v_firstDatetimeMark      TIMESTAMPTZ;  ----! TODO à supprimer qd emaj_seq_hole modifié
    v_firstMarkTimeId        BIGINT;
    v_lastMarkId             BIGINT;
    v_lastMarkGlobalSeq      BIGINT;
    v_lastDatetimeMark       TIMESTAMPTZ;  ----! TODO à supprimer qd emaj_seq_hole modifié
...
-- retrieve the id, the timestamp and the emaj_gid value of the first mark, if supplied
    IF v_firstMark IS NOT NULL THEN
      SELECT mark_id, time_last_emaj_gid, time_clock_timestamp, mark_time_id INTO v_firstMarkId, v_firstMarkGlobalSeq, v_firstDatetimeMark, v_firstMarkTimeId
        FROM emaj.emaj_mark, emaj.emaj_time_stamp
        WHERE mark_time_id = time_id AND mark_group = v_groupName AND mark_name = v_firstMark;
      ELSE
        v_firstMarkId = 0; v_firstMarkGlobalSeq = 0;
    END IF;
-- retrieve the id, the timestamp and the emaj_gid value of the last mark
    SELECT mark_id, time_last_emaj_gid, time_clock_timestamp, mark_time_id INTO v_lastMarkId, v_lastMarkGlobalSeq, v_lastDatetimeMark, v_lastMarkTimeId
      FROM emaj.emaj_mark, emaj.emaj_time_stamp
...
-- delete all existing holes (if any) between both marks
    DELETE FROM emaj.emaj_seq_hole USING emaj.emaj_relation
      WHERE rel_group = v_groupName AND rel_kind = 'r' AND rel_schema = sqhl_schema AND rel_tblseq = sqhl_table
        AND sqhl_begin_mark_id >= v_firstMarkId AND sqhl_begin_mark_id < v_lastMarkId;
    IF v_firstMark IS NOT NULL THEN
-- and if not deleting the oldest marks, create holes representing the deleted logs
...
--   the hole sizes are computed using the sequence last values recorded into the emaj_sequence table
      INSERT INTO emaj.emaj_seq_hole (sqhl_schema, sqhl_table, sqhl_begin_mark_id, sqhl_begin_mark_ts, sqhl_end_mark_id, sqhl_end_mark_ts, sqhl_hole_size)
        SELECT rel_schema, rel_tblseq, v_firstMarkId, v_firstDatetimeMark, v_lastMarkId, v_lastDatetimeMark,
...
-- if not deleting the oldest marks, set the mark_log_rows_before_next of the first mark to 0
    IF v_firstMark IS NOT NULL THEN
...
-- the numbers of log rows is computed using the _log_stat_tbl() function.
     INSERT INTO emaj.emaj_rlbk_plan
      ...
      SELECT v_rlbkId, 'RLBK_TABLE', rel_schema, rel_tblseq, '',
             emaj._log_stat_tbl(t, v_markId, NULL, v_markTimeId, NULL)
        FROM (SELECT * FROM emaj.emaj_relation
                WHERE rel_group = ANY (v_groupNames) AND rel_kind = 'r') AS t
        WHERE emaj._log_stat_tbl(t, v_markId, NULL, v_markTimeId, NULL) > 0;
...
-- process the deletion of log rows
          SELECT emaj._delete_log_tbl(emaj_relation.*, v_rlbkMarkId, v_rlbkMarkTimeId, v_timestampMark, v_genMarkId, v_rlbkStartTs, v_lastGlobalSeq, v_lastSequenceId)
            INTO v_nbRows
            FROM emaj.emaj_relation
            WHERE rel_schema = r_step.rlbp_schema AND rel_tblseq = r_step.rlbp_table;
...
--     v_realFirstMark remains NULL
      SELECT mark_id, mark_name, mark_time_id, mark_datetime INTO v_firstMarkId, v_realFirstMark, v_firstMarkTimeId, v_tsFirstMark
...
-- else, check and retrieve the name, timestamp and last sequ_hole id of the supplied first mark for the group
      ...
      END IF;
      SELECT mark_id, mark_time_id, mark_datetime INTO v_firstMarkId, v_firstMarkTimeId, v_tsFirstMark
...
-- if a last mark name is supplied, check and retrieve the name, timestamp and last sequ_hole id of the supplied end mark for the group
    IF v_lastMark IS NOT NULL AND v_lastMark <> '' THEN
      ...
      END IF;
      SELECT mark_id, mark_time_id, mark_datetime INTO v_lastMarkId, v_lastMarkTimeId, v_tsLastMark
        FROM emaj.emaj_mark WHERE mark_group = v_groupName AND mark_name = v_realLastMark;
-- if last mark is null or empty, v_realLastMark, v_lastMarkId, v_lastMarkTimeId, v_tsLastMark and v_lastLastSeqHoleId remain NULL
    END IF;
    ...
    RETURN QUERY
      SELECT rel_group, rel_schema, rel_tblseq,
             CASE WHEN v_firstMarkTimeId IS NULL THEN 0
                  ELSE emaj._log_stat_tbl(emaj_relation, v_firstMarkId, v_lastMarkId, v_firstMarkTimeId, v_lastMarkTimeId) END AS nb_rows
        FROM emaj.emaj_relation
...
--    and the mark ids are just used to check that first mark is prior the last mark)
    SELECT mark_id, mark_time_id, time_last_emaj_gid, time_clock_timestamp INTO v_firstMarkId, v_firstMarkTimeId, v_firstEmajGid, v_tsFirstMark
      FROM emaj.emaj_mark, emaj.emaj_time_stamp
...
-- else, retrieve the name, timestamp and last sequ_hole id of the supplied end mark for the 1st group
      SELECT mark_id, mark_time_id, time_last_emaj_gid, time_clock_timestamp
        INTO v_lastMarkId, v_lastMarkTimeId, v_lastEmajGid, v_tsLastMark
        FROM emaj.emaj_mark, emaj.emaj_time_stamp
    ...
    FOR r_rel IN
        SELECT * FROM emaj.emaj_relation
          WHERE rel_group = ANY (v_groupNames) AND rel_kind = 'r'                        -- tables of the groups
            ...
            AND emaj._log_stat_tbl(emaj_relation, v_firstMarkId, v_lastMarkId, v_firstMarkTimeId, v_lastMarkTimeId) > 0
          ORDER BY rel_priority, rel_schema, rel_tblseq
        LOOP
...
GRANT EXECUTE ON FUNCTION emaj._log_stat_tbl(r_rel emaj.emaj_relation, v_firstMarkId BIGINT, v_lastMarkId BIGINT, v_firstMarkTimeId BIGINT, v_lastMarkTimeId BIGINT) TO emaj_viewer;
...
INSERT INTO emaj.emaj_hist (hist_function, hist_object, hist_wording) VALUES ('EMAJ_INSTALL','E-Maj 2.0.0', 'Upgrade from 1.3.0 completed');
