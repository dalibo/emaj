...
DROP FUNCTION emaj.emaj_set_mark_group(v_groupName TEXT, v_mark TEXT);
DROP FUNCTION emaj.emaj_set_mark_groups(v_groupNames TEXT[], v_mark TEXT);
DROP FUNCTION emaj._set_mark_groups(v_groupName TEXT[], v_mark TEXT, v_multiGroup BOOLEAN, v_eventToRecord BOOLEAN);
...
-- ... record the associated sequence parameters in the emaj sequence table
    ...
    END LOOP;
-- record the marks
-- get the last id for emaj_sequence table, and the last value for emaj_global_seq
...
-- insert the marks into the emaj_mark table
    FOR v_i IN 1 .. array_upper(v_groupNames,1) LOOP
      INSERT INTO emaj.emaj_mark (mark_group, mark_name, mark_datetime, mark_global_seq, mark_is_deleted, mark_is_rlbk_protected,
                                  mark_last_sequence_id, mark_logged_rlbk_target_mark)
        VALUES (v_groupNames[v_i], v_mark, v_timestamp, v_lastGlobalSeq, FALSE, FALSE, v_lastSequenceId, v_logged_rlbk_target_mark);
    END LOOP;
...
-- Output: number of deleted marks, number of tables effectively processed (for which at least one log row has been deleted)
  DECLARE
    ...
    r_rel                    RECORD;
  BEGIN
-- retrieve the id and datetime of the first mark, if supplied
    IF v_firstMark IS NOT NULL THEN
      SELECT mark_id, mark_global_seq, mark_datetime INTO v_firstMarkId, v_firstMarkGlobalSeq, v_firstDatetimeMark
        FROM emaj.emaj_mark WHERE mark_group = v_groupName AND mark_name = v_firstMark;
      ELSE
        v_firstMarkId = 0; v_firstMarkGlobalSeq = 0;
    END IF;
-- retrieve the id and datetime of the last mark
    SELECT mark_id, mark_global_seq, mark_datetime INTO v_lastMarkId, v_lastMarkGlobalSeq, v_lastDatetimeMark
