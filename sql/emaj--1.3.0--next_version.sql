...
CREATE OR REPLACE FUNCTION emaj._delete_log_tbl(r_rel emaj.emaj_relation, v_beginMarkId BIGINT, v_beginMarkTs TIMESTAMPTZ, v_endMarkId BIGINT, v_endMarkTs TIMESTAMPTZ, v_lastGlobalSeq BIGINT, v_lastSequenceId BIGINT)
...
-- The function is called by emaj._rlbk_session_exec()
-- Input: row from emaj_relation corresponding to the appplication table to proccess,
--        begin and end mark ids and mark timestamps to define the marks range identifying the hole that will be created in the log sequence
...
-- and then insert the new sequence hole
    EXECUTE 'INSERT INTO emaj.emaj_seq_hole (sqhl_schema, sqhl_table, sqhl_begin_mark_id, sqhl_begin_mark_ts, '
      ...
      || ' AND sequ_datetime = ' || quote_literal(v_beginMarkTs) || '))';
    RETURN v_nbRows;
  END;
$_delete_log_tbl$;
...
CREATE OR REPLACE FUNCTION emaj._log_stat_tbl(r_rel emaj.emaj_relation, v_tsFirstMark TIMESTAMPTZ, v_tsLastMark TIMESTAMPTZ, v_firstMarkId BIGINT, v_lastMarkId BIGINT)
RETURNS BIGINT LANGUAGE plpgsql AS
$_log_stat_tbl$
...
-- Input: schema name and table name, log schema, the timestamps of both marks, the mark id of both marks
...
--   ... process its deletion with _delete_between_mark_group(), as the first rows of log tables become useless
      PERFORM emaj._delete_between_mark_group(v_groupName, NULL, v_markNewMin);
    ELSE
-- otherwise,
...
-- effectively delete all marks before the supplied mark
    SELECT (emaj._delete_between_mark_group(v_groupName, NULL, v_realMark)).v_nbMark INTO v_nbMark;
-- insert end in the history
    INSERT INTO emaj.emaj_hist (hist_function, hist_event, hist_object, hist_wording)
...
CREATE OR REPLACE FUNCTION emaj._delete_between_mark_group(v_groupName TEXT, v_firstMark TEXT, v_lastMark TEXT, OUT v_nbMark INT, OUT v_nbTbl INT)
...
-- Output: number of deleted marks, number of tables effectively processed (for which at least one log row has been deleted)
  DECLARE
    ...
    v_firstDatetimeMark      TIMESTAMPTZ;
    v_lastMarkId             BIGINT;
    v_lastMarkGlobalSeq      BIGINT;
    v_lastDatetimeMark       TIMESTAMPTZ;
    v_nbUpd                  BIGINT;
    r_rel                    RECORD;
  BEGIN
-- retrieve the id, the timestamp and the emaj_gid value of the first mark, if supplied
    IF v_firstMark IS NOT NULL THEN
      SELECT mark_id, time_last_emaj_gid, time_clock_timestamp INTO v_firstMarkId, v_firstMarkGlobalSeq, v_firstDatetimeMark
...
-- retrieve the id, the timestamp and the emaj_gid value of the last mark
    SELECT mark_id, time_last_emaj_gid, time_clock_timestamp INTO v_lastMarkId, v_lastMarkGlobalSeq, v_lastDatetimeMark
      FROM emaj.emaj_mark, emaj.emaj_time_stamp
      ...
      INSERT INTO emaj.emaj_seq_hole (sqhl_schema, sqhl_table, sqhl_begin_mark_id, sqhl_begin_mark_ts, sqhl_end_mark_id, sqhl_end_mark_ts, sqhl_hole_size)
        SELECT rel_schema, rel_tblseq, v_firstMarkId, v_firstDatetimeMark, v_lastMarkId, v_lastDatetimeMark,
               (SELECT CASE WHEN sequ_is_called THEN sequ_last_val + sequ_increment ELSE sequ_last_val END FROM emaj.emaj_sequence
                  WHERE sequ_schema = rel_log_schema AND sequ_name = rel_log_sequence AND sequ_datetime = v_lastDatetimeMark)
               -
               (SELECT CASE WHEN sequ_is_called THEN sequ_last_val + sequ_increment ELSE sequ_last_val END FROM emaj.emaj_sequence
                  WHERE sequ_schema = rel_log_schema AND sequ_name = rel_log_sequence AND sequ_datetime = v_firstDatetimeMark)
          FROM emaj.emaj_relation
          WHERE rel_group = v_groupName AND rel_kind = 'r'
            AND 0 <
               (SELECT CASE WHEN sequ_is_called THEN sequ_last_val + sequ_increment ELSE sequ_last_val END FROM emaj.emaj_sequence
...
-- get some mark attributes from emaj_mark
    SELECT mark_id, mark_datetime INTO v_markId, v_markDatetime
      FROM emaj.emaj_mark
...
-- the numbers of log rows is computed using the _log_stat_tbl() function.
     INSERT INTO emaj.emaj_rlbk_plan
      ...
      SELECT v_rlbkId, 'RLBK_TABLE', rel_schema, rel_tblseq, '',
             emaj._log_stat_tbl(t, v_markDatetime, NULL, v_markId, NULL)
        FROM (SELECT * FROM emaj.emaj_relation
                WHERE rel_group = ANY (v_groupNames) AND rel_kind = 'r') AS t
        WHERE emaj._log_stat_tbl(t, v_markDatetime, NULL, v_markId, NULL) > 0;
...
-- fetch the mark_id, the last global sequence and the last id values of the emaj_sequence table at set_mark time for the first group of the groups array (they all share the same values - except for the mark_id)
    SELECT mark_id, time_last_emaj_gid, mark_last_sequence_id
      INTO v_rlbkMarkId, v_lastGlobalSeq, v_lastSequenceId
      FROM emaj.emaj_mark, emaj.emaj_time_stamp
...
-- process the deletion of log rows
          SELECT emaj._delete_log_tbl(emaj_relation.*, v_rlbkMarkId, v_timestampMark, v_genMarkId, v_rlbkStartTs, v_lastGlobalSeq, v_lastSequenceId)
            INTO v_nbRows
            FROM emaj.emaj_relation
            WHERE rel_schema = r_step.rlbp_schema AND rel_tblseq = r_step.rlbp_table;
...
-- perform the consolidation operation
    SELECT * FROM emaj._delete_between_mark_group(v_groupName, v_firstMark, v_lastMark) INTO v_nbMark,v_nbTbl;
-- insert end in the history
    INSERT INTO emaj.emaj_hist (hist_function, hist_event, hist_object, hist_wording)
...
--     v_realFirstMark remains NULL
      SELECT mark_id, mark_name, mark_datetime INTO v_firstMarkId, v_realFirstMark, v_tsFirstMark
...
-- for each table of the emaj_relation table, get the number of log rows and return the statistic
    RETURN QUERY
      SELECT rel_group, rel_schema, rel_tblseq,
             CASE WHEN v_tsFirstMark IS NULL THEN 0
                  ELSE emaj._log_stat_tbl(emaj_relation, v_tsFirstMark, v_tsLastMark, v_firstMarkId, v_lastMarkId) END AS nb_rows
        FROM emaj.emaj_relation
    ...
    FOR r_rel IN
        SELECT * FROM emaj.emaj_relation
          WHERE rel_group = ANY (v_groupNames) AND rel_kind = 'r'                        -- tables of the groups
            ...
            AND emaj._log_stat_tbl(emaj_relation, v_tsFirstMark, v_tsLastMark, v_firstMArkId, v_lastMarkId) > 0
          ORDER BY rel_priority, rel_schema, rel_tblseq
        LOOP
...
GRANT EXECUTE ON FUNCTION emaj._log_stat_tbl(r_rel emaj.emaj_relation, v_tsFirstMark TIMESTAMPTZ, v_tsLastMark TIMESTAMPTZ, v_firstMarkId BIGINT, v_lastMarkId BIGINT) TO emaj_viewer;
