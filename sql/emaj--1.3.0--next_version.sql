...
-- The emaj_seq_hole structure change needs a quite complex processing.
-- the emaj_mark table must have been upgraded, but the temporary table with its old content must still exist.
DO
$do$
  DECLARE
    v_beginTimeId            BIGINT;
    v_endTimeId              BIGINT;
    v_lastTimeId             BIGINT;
    r_seq_hole               RECORD;
  BEGIN
-- get the last recorded time stamp id.
--   it will be used as end time stamp id for the specific case when a hole is produced by a rollback operation that is not followed by any mark.
    SELECT max(time_id) INTO v_lastTimeId FROM emaj.emaj_time_stamp;
-- for each existing row in the emaj_seq_hole table
    FOR r_seq_hole IN
      SELECT sqhl_id, sqhl_schema, sqhl_table, sqhl_hole_size FROM emaj_seq_hole_old ORDER BY sqhl_id
      LOOP
-- get the id of the target mark of the rollback operation that produced the hole
      SELECT max(emaj_mark.mark_time_id) INTO v_beginTimeId FROM emaj_mark_old, emaj.emaj_mark
        WHERE emaj_mark_old.mark_id = emaj_mark.mark_id
          AND emaj_mark_old.mark_last_seq_hole_id < r_seq_hole.sqhl_id;
-- get the id of the first mark set after the rollback operation that produced the hole
      SELECT coalesce(min(emaj_mark.mark_time_id),v_lastTimeId) INTO v_endTimeId FROM emaj_mark_old, emaj.emaj_mark
        WHERE emaj_mark_old.mark_id = emaj_mark.mark_id
          AND emaj_mark_old.mark_last_seq_hole_id >= r_seq_hole.sqhl_id;
-- insert the new row in the emaj_seq_hole table
      INSERT INTO emaj.emaj_seq_hole (sqhl_schema, sqhl_table, sqhl_begin_time_id, sqhl_end_time_id, sqhl_hole_size)
        VALUES (r_seq_hole.sqhl_schema, r_seq_hole.sqhl_table, v_beginTimeId, v_endTimeId, r_seq_hole.sqhl_hole_size);
    END LOOP;
  END;
$do$;
...
INSERT INTO emaj.emaj_rlbk (
  ...
  SELECT rlbk_id, rlbk_groups, rlbk_mark, t1.time_id, t2.time_id, rlbk_is_logged,
         rlbk_nb_session, rlbk_nb_table, rlbk_nb_sequence, rlbk_eff_nb_table, rlbk_status, 
         rlbk_begin_hist_id, rlbk_is_dblink_used, rlbk_start_datetime, rlbk_end_datetime, rlbk_msg
    FROM emaj_rlbk_old, emaj.emaj_time_stamp t1, emaj.emaj_time_stamp t2
    WHERE rlbk_mark_datetime = t1.time_clock_timestamp AND t1.time_event = 'M'
      AND rlbk_start_datetime = t2.time_clock_timestamp AND t2.time_event = 'R';
...
CREATE OR REPLACE FUNCTION emaj._get_mark_time_id(TEXT, TEXT)
RETURNS BIGINT LANGUAGE sql AS
$$
...
-- create the row representing the rollback event in the emaj_rlbk table and get the rollback id back
    v_stmt = 'INSERT INTO emaj.emaj_rlbk (rlbk_groups, rlbk_mark, rlbk_mark_time_id, rlbk_is_logged, ' ||
