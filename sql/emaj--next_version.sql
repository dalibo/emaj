...
-- effectively delete all marks before the supplied mark
    SELECT emaj._delete_between_mark_group(v_groupName, NULL, v_realMark) INTO v_nbMark;
-- insert end in the history
    INSERT INTO emaj.emaj_hist (hist_function, hist_event, hist_object, hist_wording)
...
CREATE OR REPLACE FUNCTION emaj._delete_between_mark_group(v_groupName TEXT, v_firstMark TEXT, v_lastMark TEXT)
RETURNS integer LANGUAGE plpgsql AS
...
-- delete log rows prior to the new first mark
      EXECUTE 'DELETE FROM ' || v_logTableName || ' WHERE emaj_gid > ' || v_firstMarkGlobalSeq || ' AND emaj_gid <= ' || v_lastMarkGlobalSeq;
    END LOOP;
...
--   the hole sizes are computed using the sequence last values recorded into the emaj_sequence table
      INSERT INTO emaj.emaj_seq_hole (sqhl_schema, sqhl_table, sqhl_begin_mark_id, sqhl_begin_mark_ts, sqhl_end_mark_id, sqhl_end_mark_ts, sqhl_hole_size)
        SELECT rel_schema, rel_tblseq, v_firstMarkId, v_firstMarkTs, v_lastMarkId, v_lastMarkTs,
               (SELECT CASE WHEN is_called THEN last_value + increment_by ELSE last_value END FROM emaj.emaj_sequence 
                  WHERE sequ_schema = rel_log_schema AND sequ_name = rel_log_sequence AND sequ_datetime = v_lastMarkTs)
               -
               (SELECT CASE WHEN is_called THEN last_value + increment_by ELSE last_value END FROM emaj.emaj_sequence 
                  WHERE sequ_schema = rel_log_schema AND sequ_name = rel_log_sequence AND sequ_datetime = v_firstMarkTs)
          FROM emaj.emaj_relation
